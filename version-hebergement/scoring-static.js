// Système de scoring avec localStorage (version statique sans serveur)
...
class ScoreManager {
    ...
    loadScores() {
        try {
            const savedScores = localStorage.getItem('superDarioBrosseScores');
            if (savedScores) {
    ...
    saveScores() {
        try {
            localStorage.setItem('superDarioBrosseScores', JSON.stringify(this.scores));
        } catch (error) {
    ...
    // Ajouter un score
    addScore(playerName, level, coins, time, health, won) {
        ...
        this.scores[category].sort((a, b) => b.points - a.points);

        // Limiter à 100 scores par catégorie pour ne pas surcharger localStorage
        if (this.scores[category].length > 100) {
            this.scores[category] = this.scores[category].slice(0, 100);
        }
        ...
        const rank = this.scores[category].findIndex(s =>
...
function displayScoreBoard(scoreManager, category = 'normal', highlightPlayer = null) {
    ...
    html += `
        ...
        <button onclick="confirmResetScores()" style="background: #DC143C; margin-left: 10px;">
...
// Confirmer la réinitialisation des scores
function confirmResetScores() {
    if (confirm('⚠️ Voulez-vous vraiment supprimer TOUS les scores ?\n\nCette action est irréversible !')) {
...
function savePlayerScore(isVictory, currentLevel, coinsCollected, gameTime, maxHealth) {
    ...
    try {
        const score = scoreManager.addScore(
            playerName,
            currentLevel,
            coinsCollected,
            gameTime,
